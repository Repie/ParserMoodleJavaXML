...
/**
 * Class open a XMLFile Check validity and wellform of XML file Check consistent
 * quiz
 * 
...
public class OpenXml implements dcll.interfaces.ConstantsTAG {
	 ...
	 * @author Pierre Rigaud
	 * @param xmlFile
	 *            as String like "Documents\\quizExamples\\malformedQuiz_1.xml"
	 */
	...
	/**
	 * Check a quiz and indicates if is not a consistent quiz create a file with
	 * intruders
	 * 
	 * @author Pierre Rigaud
	 * @throws XMLException
	 *             if XML file is not valid or badformed
	 * @throws XMLRootElementException
	 *             if the root Element is not quiz
	 * @throws XMLNoQuestionsException
	 *             if Element quiz contains another tags that question tag
	 */
	public void checkedQuiz() throws XMLException, XMLRootElementException,
			XMLNoQuestionsException {
		...
		// if the XMLfile is ok
			...
			if (!root.getName().equals(ROOT_ELEMENT))
				throw new XMLRootElementException();
			// if the root element is ok
				...
				// if no question find in quiz, indicates that thanks to
				// Exception
				if (nbQuestions == 0)
					throw new XMLNoQuestionsException();
	...
	private void createFileIntruders(String nameFile, List<Element> intruders) {
		...
		String pathFile = System.getProperty("user.dir")
				+ "\\Documents\\IntrudersXML\\Intruders_"
				+ nameFile.replace(".xml", ".txt");
		try {
			// create the FileWriter and BufferedWriter to write intruders in a
			// file
